...
export const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
  ...
  return (
    <div className="glass-panel rounded-xl border border-slate-800/60 hover:border-indigo-500/30 transition-all duration-300 overflow-hidden group">
      {/* Header / Main Row */}
      <div 
        className="p-5 cursor-pointer bg-gradient-to-r from-transparent via-transparent to-indigo-500/0 hover:to-indigo-500/5 transition-all"
            ...
            <div className="p-3 flex items-start gap-3 hover:bg-slate-800/30 transition-colors cursor-pointer" onClick={() => setOpen(!open)}>
                 ...
                 <div className="flex-1 min-w-0">
                     <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-mono text-slate-500 ml-2">#{subtask.id}</span>
                     </div>
                     <p className="text-xs text-slate-500 truncate mt-0.5">{subtask.description}</p>
                 </div>
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        animate={{ height: 'auto' }}
                        exit={{ height: 0 }}
                        className="bg-slate-950/50 border-t border-slate-800/50 text-xs p-3 space-y-2"
                    >
                        {parsed && parsed.logs.length > 0 && (
                            <div className="mb-2 bg-indigo-950/30 text-indigo-200 p-2 rounded border border-indigo-900/50 font-mono">
                                {parsed.logs.length} Updates logged. Check main details.
                            </div>
                        )}
                         <p className="text-slate-400">{subtask.details || "No details."}</p>
                         {subtask.testStrategy && (
                             <div className="text-slate-500 italic border-l-2 border-slate-700 pl-2">
                                 Test: {subtask.testStrategy}
                             </div>
                         )}
                    </motion.div>
                )}
